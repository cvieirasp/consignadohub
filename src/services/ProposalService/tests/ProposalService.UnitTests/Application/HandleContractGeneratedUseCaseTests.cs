...
public sealed class HandleContractGeneratedUseCaseTests
{
    ...
    private readonly HandleContractGeneratedUseCase _useCase;

    public HandleContractGeneratedUseCaseTests()
    {
        _useCase = new HandleContractGeneratedUseCase(
            _proposalRepositoryMock.Object,
            _inboxRepositoryMock.Object);
    }

    private static ContractGeneratedEvent MakeEvent(Guid proposalId) =>
        new(proposalId, Guid.NewGuid(), "https://contracts.consignadohub.internal/abc.pdf");

    [Fact]
    [Trait("Category", "Unit")]
    public async Task Execute_ShouldTransitionToContractGenerated_WhenProposalIsApproved()
    {
        var proposalId = Guid.NewGuid();
        var proposal = CreateApprovedProposal(proposalId);
